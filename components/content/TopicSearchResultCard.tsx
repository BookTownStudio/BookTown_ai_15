...
import type { SocialSearchTopic } from '../../lib/hooks/useSocialSearch.ts';

interface TopicSearchResultCardProps {
    topic: SocialSearchTopic;
}

const TopicSearchResultCard: React.FC<TopicSearchResultCardProps> = ({ topic }) => {
    const postsLabel = topic.postCount === 1 ? '1 post' : `${topic.postCount.toLocaleString()} posts`;
    ...
    return (
        <div className="p-4 flex items-center gap-3 border-b border-black/10 dark:border-white/10">
            ...
            <div>
                <BilingualText className="font-bold">{topic.topic}</BilingualText>
                <BilingualText role="Caption">{postsLabel}</BilingualText>
            </div>
        </div>
    );
};
