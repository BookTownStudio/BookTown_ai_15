    ...
    try {
        const normalizedContextMessages = contextMessages
            ...
            .map((msg) => ({
                ...
                text: msg.text.replace(/\s+/g, ' ').trim().slice(0, 2000),
            }))
        ...
        const messages = normalizedContextMessages.map(msg => ({
            role: msg.role,
            content: msg.text
        }));

        const responseText = await agentService.discoverAgentChat(agentId, messages, systemInstruction);
