...
const searchBookSchema = z
  .object({
    ...
    workType: z.enum(["work", "edition"]),
    editionPresence: z.enum(["single", "grouped", "edition"]),
    ebookClass: z.enum(["in_app", "external_link", "unavailable"]),
    sourceClass: z.enum(["canonical_catalog", "external_provider"]),
    languageTruth: z.enum(["match", "mismatch", "unknown"]),
    ...
    listSocialFeed: defineContract(
      ...
      z
        .object({
          ...
          meta: z
            .object({
              assemblyMs: z.number().nonnegative(),
              fallbackEntityHydrationRequests: z.number().int().nonnegative(),
              fallbackEntityHydrationReads: z.number().int().nonnegative(),
              fetchAttempts: z.number().int().nonnegative().optional(),
              filteredPostCount: z.number().int().nonnegative(),
              followingAuthorReads: z.number().int().nonnegative(),
              hydrationMs: z.number().nonnegative(),
              legacyViewerStateFallbackReads: z.number().int().nonnegative(),
              legacyViewerStateFallbackRequestedCount: z.number().int().nonnegative(),
              maxFetchAttempts: z.number().int().nonnegative(),
              postDocumentsRead: z.number().int().nonnegative(),
              primaryEntityPostCount: z.number().int().nonnegative(),
              projectionAvailableCount: z.number().int().nonnegative(),
              projectionMissingCount: z.number().int().nonnegative(),
              projectedEntityHydrationHits: z.number().int().nonnegative(),
              projectedEntityHydrationRate: z.number().min(0).max(1),
              projectedViewerStateReads: z.number().int().nonnegative(),
              projectionUsageRate: z.number().min(0).max(1),
              queryBatches: z.number().int().nonnegative(),
              queryMs: z.number().nonnegative(),
              returnedPostCount: z.number().int().nonnegative(),
              statsRequestedCount: z.number().int().nonnegative(),
              statsReads: z.number().int().nonnegative(),
              unresolvedHydrationCount: z.number().int().nonnegative(),
              viewerStateProjectedHitCount: z.number().int().nonnegative(),
              viewerStateProjectionHitRate: z.number().min(0).max(1),
              viewerStateRequestedCount: z.number().int().nonnegative(),
              candidateReads: z.number().int().nonnegative().optional(),
              fetchedDocs: z.number().int().nonnegative().optional(),
            })
            .strict()
            .optional(),
        })
